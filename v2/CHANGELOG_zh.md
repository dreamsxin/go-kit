...
- 实例快照去重现在可以安全比较切片、map、interface 和不可比较的 error 值，不再因为动态值不可比较而 panic。
- 当选择次数覆盖整个快照时，最少请求选择现在执行真正的完整扫描，不会因为重复随机采样漏掉最低负载实例。
- `sd/retry` 把单次尝试的调用改为在调用方 goroutine 内联执行，不再为一个永不可能重试的调用启动尝试
  goroutine 与缓冲 channel。默认的一次尝试配置在热路径上只付出调用本身的开销；结果形态、Done 上报、
  callback 的 Final 以及预算耗尽不再派发等语义保持不变。当失败恰好与预算到期同时发生时，仍如实上报该
  尝试及其错误类型，而不是用裸的 context 错误掩盖它。
...
- **`retry.Error` 现在实现了 `Is` 与 `As`，于是一次带重试的失败里每一个原因都可达。** `Unwrap` 只能给出
  一个，而 `Final` 一旦设置就优先：预算到期之后唯一可达的原因是 `context.DeadlineExceeded`，于是上游的
  `apperror` kind——它被记在每一次尝试里——对 `errors.As` 不可见，按 kind 分发的映射器会为一个"真实类别
  一直在手上"的失败发出笼统的超时。`Callback` 的替换错误也以同样方式遮住了真正的错误。`errors.Is` 与
  `errors.As` 会在走 unwrap 链之前先咨询这两个方法，所以各次尝试和最终原因都可达，且最近的尝试在前。
  把 `Unwrap` 改成返回 `[]error` 本可以达到同样效果——代价是破坏一个已发布的签名；兼容性门禁拒绝了它，
  而它拒绝得对。
...
- **并发应用摘除策略时，被摘出服务的比例可以超过上限。** `feedback.Ejector` 在锁外判定候选、在锁外
  检查 `MaxEjectionPercent`，然后重新加锁去应用；于是四实例池上的两次 pick 可以各自基于"不包含对方
  决定"的视图通过 50% 的检查并一起摘除——池子的 75% 消失，而这正是那个上限存在的目的所要防止的情形，
  同时 panic 模式根本不会触发。同一个窗口还让两次 pick 摘除**同一个**实例，而摘除是被计数用来加长窗口的，
  于是"第一次犯错"被记成两次，实例被摘出的时长变成配置基准的两倍。现在判定与应用在同一个临界区里完成；
  测量值在进入临界区之前读取，因为它们来自 `Table` 自己的互斥锁，而 `Table.Follow` 会反方向驱动
  `Ejector.Retain`。
- **服务发现的一次抖动会把正在失败的实例复活成健康。** `health.Checker` 会删掉缺席地址的状态，并在它
  返回时用 `initiallyHealthy`（默认 true）和"零条失败记录"重建，于是刚刚被它摘出服务的后端被重新发布为
  可服务，并且在 `unhealthyThreshold` 轮探测之内无法再被摘除。注册中心是会抖的：心跳 TTL 早一拍过期、
  分页或不完整的列表、provider 短暂只报了一个子集。现在状态能扛过一次缺席，在第二次缺席时才丢弃——抖动
  不花代价，而真正的离开仍然会被忘掉。要注意：会抑制"与上一次完全相同的事件"的来源（`instance.Cache`
  就是）只有在集合再次变化时才会送出那第二次缺席。
- **并发更新会让订阅者手里的实例列表比缓存自己报告的更旧。** `instance.Cache.Update` 在锁内写入新状态，
  在释放锁之后才广播；于是两个同时更新的调用方可以按 A、B 的顺序写入状态，却按 B、A 的顺序送达。
  `sendLatest` 不但没救回来还加重了它：发现缓冲已满时它会排掉再写入，于是较旧的事件把较新的替换掉了。
  此后 `State` 与每一个订阅者不一致，直到下一次更新；而由于"与已存状态相同的事件"会被丢弃，陈旧列表
  再来一次并不会纠正它——请求会继续发往一个已经被注销的地址。现在广播在锁内完成，这是安全的：
  `sendLatest` 里每一个 channel 操作都有 default 分支，不会因为某个慢订阅者而阻塞。
- **同两次审计里的另外三处较小问题。** 加权选择在求和时不设上界，于是两个注册权重接近 `MaxInt` 的实例
  会让总和绕成负数，一个完全健康的池子报出 `ErrNoEndpoints`——而它被归类为临时错误，所以调用方会先把
  重试预算烧在它上面；现在权重会被夹住，使总和不可能溢出。`endpointer.Filter` 与 `Prefer` 接受 nil
  source 并把 panic 推迟到第一个请求，而它所有的同类构造函数都在装配时校验。以及 `feedback.Follow`
  按引用保留了 provider 的实例切片，而在完全相同的边界上 `health.accept` 做了复制并写明了原因——会重用
  底层数组的 provider 可以改写一份已经交给 retainer 的集合。
