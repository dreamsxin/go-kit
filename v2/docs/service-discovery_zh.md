...
所以在退避期间超时的调用不会再发出一次上游请求。

此时的失败是一个 `retry.Error`，携带至此为止的全部尝试记录，`Final` 为 context 错误，
因此 `errors.Is(err, context.DeadlineExceeded)` 仍然匹配，而错误消息会指出失败的实例。
`retry.Error` 还实现了 `Is` 与 `As`，所以 `errors.As` 能触达任一尝试里上游报告的类别——
预算到期不再把 `apperror` 藏在 context 错误后面。当预算在"任何一次尝试的结果被取走之前"结束调用时，
返回的是裸的 context 错误；而在同一瞬间到达的错误有意不被记为一次尝试，否则"两者谁先到"就会决定
错误的形状。
