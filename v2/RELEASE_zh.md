...
v2.3.0 是当前从 `main` 为以下独立模块发布的版本：

```text
github.com/dreamsxin/go-kit/v2
```

`v2.3.0` 向后兼容：仅包含增量能力与行为修复，没有新的 SemVer 例外。两个历史例外仍记录在 [CHANGELOG_zh.md](CHANGELOG_zh.md) 和 [MIGRATION_zh.md](MIGRATION_zh.md) 中：`v2.1.0` 的直接重构，以及 `v2.2.0` 的 `Metrics.Snapshot()` 返回类型修正。本次发布的能力范围记录为 [ROADMAP_zh.md](ROADMAP_zh.md) 中的里程碑 7。

已发布模块存储在仓库的 `v2` 主版本子目录中，但使用方请求的是正常的模块版本，例如 `v2.3.0`。它的标签是根标签 `v2.3.0`，而不是 `v2/v2.3.0`。未来的 `/v3` 模块同样会使用根 `v3.0.0` 标签。v1 发布历史仍可通过不可变的 v1 标签获取，不会在 `main` 上重复保留。

不可变的 `v2.0.0`、`v2.1.0` 和 `v2.2.0` 发布仍然可用。本次发布将发布根 `v2.3.0` 标签，并为全部八个独立版本化的嵌套模块发布新的 `v0.2.1` 标签：`microgen` 修复失效的 `-add-tables` 标志，七个可选模块把核心依赖更新为 `v2.3.0`。
...
- 根运行时：`github.com/dreamsxin/go-kit/v2@v2.3.0`；
- microgen 和可选嵌套模块：独立的 `v0.2.1` 发布（`microgen` 为失效的 `-add-tables` 标志修复；可选模块为核心依赖更新到 `v2.3.0`）；
- 示例和仓库工具：不作为产品模块发布。
