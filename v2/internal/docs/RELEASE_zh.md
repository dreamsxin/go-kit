...
v2.7.0 是本次从 `main` 为以下独立模块发布的版本：

```text
github.com/dreamsxin/go-kit/v2
```

`v2.7.0` 是经批准的错误契约版本：一套分类同时服务两个方向，因此客户端调用与服务端
endpoint 共用同一套中间件与同一批映射表。它包含经批准的破坏性变更：本地准入拒绝返回
503 而非 429、`RateLimiterFunc` 改名为 `RateLimiterFuncs`、转发的客户端错误保持上游
状态码与业务码而不再退化为 500。嵌套模块发布 `v0.4.0` 标签；从嵌套候选提交起，依赖
核心的模块要求 `v2.7.0`。早期版本的历史记录（包括已记录的 SemVer 例外）见
