...
v2.23.0 是本次从 `main` 准备的候选版本，新增可配置的重试退避与时钟，并包含此前未发布补丁候选中的
服务发现和手动定时器修复。新增公开 API 使用 minor 版本；v2.22.0 仍是最新已发布版本。
...
v2.23.0 候选版本打标签前必须满足以下条件：

- 兼容性契约覆盖的每个表面都指名了保证它的门禁，v2 承诺的协议行为都声明在守护它们的
  代码旁边。
- MCP 表面同时服务当前规范修订与它之前的一版，因此不会在一个过期的协议上宣布冻结。
- `kit`、endpoint、HTTP/gRPC 传输、服务发现和交互生命周期都具有明确的错误与取消契约。
- 停止是一个被声明的过程，而不是一场竞速：readiness 失败与离开服务发现都发生在 drain 延迟
  之前，且没有任何关闭路径会在它自己拥有的连接仍然打开时返回。
- "停机做不到什么"和"做得到什么"一样被明确写出：被 hijack 的连接不会被 drain，而拥有它的
  那个升级 handler 会收到停止信号。
- 两个服务传输回答同一组运维问题——就绪、draining 通告、停止信号、停机预算、TLS、指标、
  tracing——否则差异被声明出来并有门禁看住。
- 证书可以在进程服务期间被替换，替换失败时继续服务上一张，而文档写明了还有哪些只读一次。
- 监听器可以按配置终止 TLS，库与生成代码都可以；无法加载的证书会带着路径让启动失败；随
  TLS 一起到来的协议也被写明——经 ALPN 的 HTTP/2、流式不受影响、不提供明文 h2c。
...
- CSRF token 绑定到会话身份与有效期，每个面向浏览器的判定都声明自己的方案与缓存范围。
- 基准覆盖每个请求都会穿过的路径；为性能而做的改动在 `CHANGELOG.md` 里记录实测效果——
  包括那些被测量否掉的。
- 就绪与存活属于任何传输都能挂载的探针注册表，因此纯 gRPC 装配拥有与 HTTP 相同的运维面。
- OpenTelemetry 一次调用完成装配，instrument 命名遵循语义约定，trace context 无需额外
  接线就穿过每种传输。
- 能编译的发现装配就是能工作的装配：一套订阅状态机服务所有消费者，依赖测量的均衡器无法
  在缺少喂养它的统计的情况下被构造出来。
- JSON 服务端用协议层面的答案回答协议层面的问题：读取 body 之前就返回 415，解码失败给出
  写给调用方看的消息。
- 生成代码把类型不匹配报成可分类的错误而不是 panic。
