...
v2.8.0 是本次从 `main` 为以下独立模块发布的版本：

```text
github.com/dreamsxin/go-kit/v2
```

`v2.8.0` 是经批准的"暴露面与韧性"版本：调用方能读到什么由一条规则在一个地方决定，
而依赖的好坏取决于它多久失败一次、应答有多慢，而不是恰好有多少次失败连在一起。它包含
经批准的破坏性变更：500 响应不再携带错误文本、`endpoint.Failer` 生效、`NewStrict*`
系列 JSON 构造器改名为 `*WithBodyLimit`、`selector.ScoreFunc` 收到请求、已关闭的
selector/balancer 返回 `sd.ErrClosed`、熔断器不再把调用方取消计为依赖失败。嵌套模块
发布 `v0.5.0` 标签，例外是 `integrations/etcd`，它的首个版本是 `v0.1.0`，本次发布
`v0.2.0`；从嵌套候选提交起，依赖核心的模块要求 `v2.8.0`。早期版本的历史记录（包括
已记录的 SemVer 例外）见 [CHANGELOG_zh.md](../../CHANGELOG_zh.md)。

已发布模块存储在仓库的 `v2` 主版本子目录中，但使用方请求的是正常的模块版本，例如 `v2.4.0`。它的标签是根标签 `v2.4.0`，而不是 `v2/v2.4.0`。未来的 `/v3` 模块同样会使用根 `v3.0.0` 标签。

`v2.4.0` 发布推送了根标签 `v2.4.0`，并为全部八个独立版本化的嵌套模块推送 `v0.2.2` 标签。`v0.2.2` 发布携带核心依赖升级到 `v2.4.0`：标签从嵌套候选提交创建，其中每个核心依赖模块都已要求 `v2.4.0`。已发布的 `v0.2.1` 可选模块仍是保留 `v2.2.0` 依赖的版本提升发布（保持兼容）。
...
- `v2.7.0` 的错误契约统一（本地准入拒绝返回 503、`RateLimiterFunc` 改名为
  `RateLimiterFuncs`、转发的客户端错误保持上游状态码与业务码）；
- `v2.8.0` 的暴露面与韧性修正（500 响应不再携带错误文本、`endpoint.Failer` 生效、
  `NewStrict*` 系列 JSON 构造器改名为 `*WithBodyLimit`、`selector.ScoreFunc` 收到请求、
  已关闭的 selector/balancer 返回 `sd.ErrClosed`、熔断器不再把调用方取消计为依赖失败）。
