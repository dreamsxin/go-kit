...
## 里程碑 24（进行中）：服务发现边界保留信息

目标：v2.23.0 候选版本，包含此前未发布的补丁工作和下述增量重试 API。已发布的 v2.22.0 保持不可变。

后续审阅发现了服务发现和定时器所有权中的缺陷：

- `sd/health` 必须在探测周期之间保留源错误。只有注册中心才能宣告自身故障结束，探测成功不能替代它。
  回归覆盖正常健康、fail-open、fail-closed 三种视图与两个下游缓存实现，验证失效、恢复和未启用失效策略时的
  旧视图保留。恢复旧的发布行为后，三个用例都会因故障期间 `err=<nil>` 而失败。
- `sd/instance` 必须能比较快照契约允许的动态值。切片、map、含接口字段的结构体及以切片为底层类型的 error，
  均复现了 Update 持锁期间的 panic。metadata 保持深度比较；可比较 error 保持身份比较，其他 error 使用深度比较；
  测试验证去重和变化通知。
- `sd/selector` 必须兑现 `WithChoices` 的完整扫描承诺。放回式随机采样在公开 API 的 500 次复现调用中，
  有 159 次漏掉了空闲成员。测试验证采样次数覆盖整个池时，每个成员只读一次、选择最低负载，同最低负载随机打散。
- `endpoint.ManualClock` 把已停止的定时器保留到原截止时间，取消的重试会累积等待，`Pending()` 也会多报。
  Stop 现在持时钟锁立即移除定时器，时间推进时清理已完成切片项。测试在旧实现上复现了仅一个有效等待却报告
  1,001 个待处理定时器，另覆盖重试取消和 Stop/Advance 并发。本次修正无需新增时钟 API。

验收：专项回归必须在旧行为下失败、修复后通过；经过评审的行为清单、API 兼容检查和已提交树上的 `make verify`
必须通过。公开签名和包路径保持兼容。

职责审阅与有意保留的设计：

- 保持 `health`（本进程存活/就绪）与 `sd/health`（远端实例探测）分离，两者回答的问题不同。
- 快照所有权归 `sd/instance`，连接创建与关闭归 `sd/endpointer`，选择归 `sd/selector`，尝试执行归 `sd/retry`，
  旧视图宽限期归共享订阅状态机。发现出错时，消费方按自己的策略继续使用上次成功视图直至失效；探测结果不是新的
  注册中心快照。本次修复保留这一既有策略。
- `feedback.Table` 与 `Ejector` 分离测量和决策；`Measured` 与 `sd/client` 提供基于这些原语的可选装配。
  不需要引入全局组件注册表或额外 module。
- `ARCHITECTURE.md` 的数字分层图已替换为包职责和明确的导入方向，补全 `health`，说明可选 gRPC 装配与
  生成依赖，并区分包构建隔离和单一 module 的依赖元数据。
- 里程碑 23 延后的重试时钟工作已通过 `retry.New(balancer, opts...)` 实现。现有入口保留签名和旧策略；
  新入口默认只尝试一次、不增加截止时间。次数上限独立于回调的继续决定。

### 可配置重试时间

`WithClock` 使用现有 `endpoint.Clock` 创建退避定时器，`WithBackoff` 接收已完成的尝试编号。
每次调用拥有自己的时间表，共享端点不会共享进度。未显式替换时保持原有随机退避；自定义非正延迟表示
立即重试，仍受取消与次数上限约束。

时钟只控制等待。整体 context 截止时间与测量的尝试耗时使用真实时间，手动时钟不能关闭生产超时或改写
遥测数据。应用回调必须及时返回，执行器不能强制停止任意应用代码。Balancer 所有权仍由调用方持有。

测试无需真实等待就能推进小时级退避，并验证默认退避累积及跨调用重置、取消释放、冻结时钟下的真实截止时间、
并发调用、测量耗时和三个旧入口。故意禁用时钟选项时，配置等待的测试会失败。新增公开声明与行为标记需要
评审快照；候选因新增能力切换为 minor 版本，本次不打新标签。

### 客户端重试装配

高层客户端现通过 `WithRetryClock` 和 `WithRetryBackoff` 转发重试时间配置，默认值、次数上限、分类和资源
所有权保持既有客户端选项语义。测试使用真实的内存发现源，在手动等待期间替换实例，验证重试命中新端点；
另覆盖取消、真实超时、nil 重置和清理。禁用时钟转发会使集成测试失败。

构造校验同时拒绝带类型的 nil balancer。此前已发布的路径接受它，而且即便普通 nil 分支也隐藏清理错误。
回归检查返回前每个端点 closer 与订阅均已释放、发现源未关闭、清理失败仍然可达。

### 发布状态门禁

最初的状态门禁只要求文档任意位置出现候选版本，另一个位置出现“candidate”，所以发布流程文字也能让错误的
当前发布声明通过。加强后的门禁检查各文档的状态章节，要求版本与阶段在同一条声明中，忽略代码块示例。
...
- 公共库表面由构造保证被文档覆盖：一道受评审的门禁会拒绝任何缺少 doc 注释的导出顶层声明，因此新增的导出名不会带着“无人解释”发布。方法、internal 包、testdata 与生成的 proto 文件按门禁所述原因豁免。此前 31 个无文档的导出标识符（发现提供者的选项类型、gRPC 传输钩子、交互事件词表等）现已补足文档，评审过的 API 快照也记录了这次变更。
...
## 里程碑 23（完成）：服务发现子树为自己作答

目标：`sd` 从未被审计过。两次审计——负载均衡路径（`balancer`、`selector`、`endpointer`、`instance`）
与韧性路径（`retry`、`feedback`、`health`、`client`）——逐条问"每种情形下部署方损失什么"，答案按
严重程度依次处理完毕。

作为 v2.22.0 发布。六个工作包：带重试的调用不丢已收到的答案；在并发下仍然成立的摘除上限；一次抖动
不等于一个新实例；订阅者不会落在已存状态之后；三处较小的发现；以及一次带重试的失败的每个原因都可达。
有一项带着设计方案有意留开——`sd/retry` 的时钟接缝需要的是一个选项式构造函数，而不是第五个位置参数。
...
已在里程碑 24 中通过选项式构造入口与可控退避测试解决；下面保留它在 v2.22.0 发布时被延后的原因。
...
- 修它需要的是一个 API 决定，而不只是一个接缝。那三个构造函数是位置参数式的（`Retry`、`WithCallback`、
