...
`docs/openapi.json` is the generated OpenAPI 3.1 contract. The running service exposes it at `GET /openapi.json` and serves Swagger UI at `GET /swagger/`. Both files under `docs/` are refreshed by generation and extend mode; do not hand-edit them.
