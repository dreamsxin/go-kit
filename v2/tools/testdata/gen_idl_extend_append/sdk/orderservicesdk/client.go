...
func (c *httpClient) do(ctx context.Context, method, path string, reqBody, respBody interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("orderservicesdk: resolve request URL: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("orderservicesdk: read response: %w", err)
	}
	if int64(len(raw)) > c.maxResponseBodyBytes {
		return fmt.Errorf("orderservicesdk: response body exceeds %d bytes", c.maxResponseBodyBytes)
	}
