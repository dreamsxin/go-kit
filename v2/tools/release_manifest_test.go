...
func TestReleaseManifestMatchesRepository(t *testing.T) {
	...
	if manifest.Phase == "candidate" {
		changelogStatus = "发布候选"
	}
	assertVersionText(t, filepath.Join(root, "CHANGELOG_zh.md"), "## ["+changelogVersion+"] - "+changelogStatus)
}
...
func TestReleaseStatusMatchesRepository(t *testing.T) {
	...
	for _, doc := range releaseStatusDocuments {
		t.Run(doc.path, func(t *testing.T) {
			...
			if err := checkReleaseStatus(string(data), doc, manifest.Phase, manifest.CoreVersion); err != nil {
