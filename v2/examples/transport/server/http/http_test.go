...
import (
	...
	transportserver "github.com/dreamsxin/go-kit/v2/transport/http/server"
)

// go test -v -count=1 -run TestServerBadDecode .\http_test.go
func TestServerBadDecode(t *testing.T) {
	handler := transportserver.NewServer(
		endpoint.Nop, // Endpoint
		func(context.Context, *http.Request) (interface{}, error) { return struct{}{}, errors.New("dang") }, // DecodeRequestFunc
		transportserver.NopResponseEncoder, // EncodeResponseFunc
	)
...
func TestServerBadEndpoint(t *testing.T) {
	handler := transportserver.NewServer(
		func(context.Context, interface{}) (interface{}, error) { return struct{}{}, errors.New("dang") }, // Endpoint
		transportserver.NopRequestDecoder,  // DecodeRequestFunc
		transportserver.NopResponseEncoder, // EncodeResponseFunc
	)
	server := httptest.NewServer(handler)
	defer server.Close()
...
func TestServerBadEncode(t *testing.T) {
	handler := transportserver.NewServer(
		endpoint.Nop,
		transportserver.NopRequestDecoder,
...
func TestServerErrorEncoder(t *testing.T) {
	...
	handler := transportserver.NewServer(
		...
		transportserver.NopRequestDecoder,  // DecodeRequestFunc
		transportserver.NopResponseEncoder, // EncodeResponseFunc
		transportserver.ServerErrorEncoder(func(_ context.Context, err error, w http.ResponseWriter) { w.WriteHeader(code(err)) }),
	)
	server := httptest.NewServer(handler)
	defer server.Close()
	resp, _ := http.Get(server.URL)
...
func TestMultipleServerBefore(t *testing.T) {
	...
	handler := transportserver.NewServer(
		...
		transportserver.ServerBefore(func(ctx context.Context, r *http.Request) context.Context {
...
func TestMultipleServerAfter(t *testing.T) {
	...
	handler := transportserver.NewServer(
		...
		transportserver.ServerAfter(func(ctx context.Context, r *http.Request, w *transportserver.InterceptingWriter) context.Context {
			ctx = context.WithValue(ctx, "one", 1)
...
func TestEncodeJSONResponse(t *testing.T) {
	handler := transportserver.NewServer(
		...
		transportserver.EncodeJSONResponse,
	)
...
func (m multiHeaderResponse) Headers() http.Header {
...
func testServer(t *testing.T) (step func(), resp <-chan *http.Response) {
	var (
		...
		handler  = transportserver.NewServer(
			...
			transportserver.ServerBefore(func(ctx context.Context, r *http.Request) context.Context { return ctx }),
			transportserver.ServerAfter(func(ctx context.Context, r *http.Request, w *transportserver.InterceptingWriter) context.Context {
				return ctx
			}),
		)
