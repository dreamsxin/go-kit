...
决定 TLS 在哪里结束，并把这个决定写下来。两处都支持，默认是代理。

在进程前面终止——sidecar、ingress、负载均衡——适用于：平台已经在轮换证书；多个服务共用
一个域名；mTLS 与加密套件策略属于网格。此时进程在只有代理能到达的网络上服务明文，而
"只有代理能到达"就是你接受的那个假设；把它写进部署，而不是留在读者脑子里。

在进程内终止适用于：到它的这一跳不可信；某条合规要求点的是这个服务而不是边缘；或者根本
没有代理——一台 VM 上的单个二进制，或一个不能是明文的 admin 端口：
...
两个键要么都设，要么都不设：只设一个会导致校验失败，而不是在客户端马上要讲 TLS 的端口上
悄悄服务明文。证书对在监听器开始服务之前加载，所以路径写错或私钥不匹配会带着路径让启动
失败。最低版本是 TLS 1.2。加密套件、客户端证书、SNI、以及不重启的轮换属于策略——
`kit.WithTLSConfig` 接收你自己构造的 `tls.Config`；生成的服务里，同样的 config 由你在
`cmd/main.go` 中设置。
...
- 两种方案下"被 hijack 的连接都不会被 drain"——见生命周期一节的 drain 说明。在进程内终止
  不改变这一点，它只是把"有升级连接"这个选项去掉了。

明文 HTTP/2（h2c）不提供。如果前面的代理想用 h2 连后端，请把它配成讲 HTTP/1.1，或者给后端
一张证书、让 ALPN 去做这件事。
