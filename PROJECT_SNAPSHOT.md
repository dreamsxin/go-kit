...
The repository is in a post-`v1.6.0` release state.
...
- `transport/http/server.JSONErrorEncoder` now emits a stable `code` field, client-facing `message`, and optional request ID, with unclassified 5xx messages redacted to status text;
...
- `transport/http/server.NewJSONServer`, `NewJSONEndpoint`, and `DecodeJSONRequest` now use strict JSON decoding by default;
